...
In addition to the 4 types above, `tx_schema!` also generates a specialised
prepared transaction builder and transaction for the schema, plus a
`prepared_tx` method on the schema type, so prepared transactions are easy to
store for later use:
...
Both the builder and the transaction types are generated by `tx_schema!` and
are also available as associated types of the schema's `TxSchema`
implementation (`TxMap::prepared_tx` returns `SCHEMA::PreparedTxBuilder`).
There is no standalone generic `PreparedTransaction`/`PreparedTxBuilder` type;
every schema gets its own specialised versions.
