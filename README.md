...
// The example below will create 6 types to use in the transaction
// 1. Transfer: Contains constants for SCHEMA and all key handles used in the transaction, (from and to)
// 2. TransferBuilder: The transaction builder type, call into_transaction() to create the re-usable transaction
// 3. TransferTx: The re-usable transaction type, generic over the map's key, value, lock policy and hasher types
// 4. TransferKeys: Used to pass in the actual keys for each execution
// 5. TransferParams: Used to pass in the real parameters for each execution
// 6. TransferState: Used by the transaction to store state and is returned as part of the final result
...
let transfer_tx = transfer_tx_builder.into_transaction();
