...
The goal is for `TxMap` to be a drop-in replacement for
[`std::collections::HashMap`](https://doc.rust-lang.org/std/collections/struct.HashMap.html).
This document compares the two API surfaces: what is the same, what is different,
and what is available in std but not in `TxMap`.
...
Because `TxMap` is internally mutable and lock-based, it can never return
references into its storage: a lock guard is dropped when the method returns, so
a `&V` or `&mut V` would dangle. Every std method that returns a reference has a
closure-based counterpart in `TxMap` (see *Different* below).

## Same API (same name, same or equivalent semantics)

| std `HashMap` | `TxMap` | Notes |
|---|---|---|
| `new()` | `new()` | |
| `Default` | `Default` | |
| `insert(k, v) -> Option<V>` | `insert(k, v) -> Option<V>` | Returns the previous value |
| `remove(&k) -> Option<V>` | `remove(&k) -> Option<V>` | |
| `remove_entry(&k) -> Option<(K, V)>` | `remove_entry(&k) -> Option<(K, V)>` | |
| `contains_key(&k) -> bool` | `contains_key(&k) -> bool` | |
| `len() -> usize` | `len() -> usize` | |
...
| `keys() -> &K` | `keys() -> &K` | |
| `values() -> &V` | `values() -> &V` | |
| `drain() -> (K, V)` | `drain() -> (K, V)` | Dropping the iterator mid-way removes all remaining entries in both |
| `capacity() -> usize` | `capacity() -> usize` | `TxMap`'s is approximate (see *Different*) |
| `reserve(additional)` | `reserve(additional)` | Distributed evenly across shards |
| `try_reserve(additional)` | `try_reserve(additional)` | |
| `shrink_to_fit()` | `shrink_to_fit()` | |
| `shrink_to(min_capacity)` | `shrink_to(min_capacity)` | |
| `hasher() -> &S` | `hasher() -> &S` | |
| `IntoIterator` (owned, `&`, `&mut`) | `IntoIterator` (owned, `&`, `&mut`) | Owned iteration is eager in `TxMap` |
| `Extend<(K, V)>`, `Extend<(&K, &V)>` | `Extend<(K, V)>`, `Extend<(&K, &V)>` | |
| `FromIterator<(K, V)>` | `FromIterator<(K, V)>` | |
| `From<[(K, V); N]>` | `From<[(K, V); N]>` | |
| `Clone` | `Clone` | Both require `V: Clone` |
| `PartialEq`, `Eq` | `PartialEq`, `Eq` | Element-wise comparison, order-independent |
| `Debug` | `Debug` | |
| serde `Serialize`/`Deserialize` | serde `Serialize`/`Deserialize` | Via the `serde` feature |
