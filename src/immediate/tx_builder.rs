    ...
    STATE: Default + 'tx,
{
    pub fn require(
        ...
        let guard = Guard {
            ...
            key: Indexer::indexed_key(
                self.custodian.shard_count,
                key,
                &self.custodian.hash_builder,
            ),
    ...
    STATE: Default + 'tx,
{
    ...
    {
        let op = MoveValueOp {
            key_from: Indexer::indexed_key(
                self.custodian.shard_count,
                key_from,
                &self.custodian.hash_builder,
            ),
            key_to: Indexer::indexed_key(
                self.custodian.shard_count,
                key_to,
                &self.custodian.hash_builder,
            ),
    ...
    {
        let op = SwapValueOp {
            key_a: Indexer::indexed_key(
                self.custodian.shard_count,
                key_a,
                &self.custodian.hash_builder,
            ),
            key_b: Indexer::indexed_key(
                self.custodian.shard_count,
                key_b,
                &self.custodian.hash_builder,
            ),
