use crate::{
    custodian::Custodian, key::TxKey, lock_policies::lock_policy::LockPolicy,
    new_types::ShardIndex, shard::Shard,
};
...
impl<'ex, K, V, L> Drop for LockGuard<'ex, K, V, L>
where
    K: 'ex,
    V: 'ex,
    L: LockPolicy + 'ex,
...
impl<'ex, K, V, L> LockGuard<'ex, K, V, L>
where
    K: 'ex,
    V: 'ex,
    L: LockPolicy + 'ex,
{
    ...
    pub fn read_shard(&self, shard_index: ShardIndex) -> &Shard<K, V> {
        ...
        unsafe { &*L::as_ptr(&self.custodian.shards[shard_index.0 as usize]) }
    }
    ...
    pub fn write_shard(&mut self, shard_index: ShardIndex) -> &mut Shard<K, V> {
        ...
        unsafe { &mut *L::as_mut_ptr(&self.custodian.shards[shard_index.0 as usize]) }
    }
...
impl<'ex, K, V, L> Deref for LockGuard<'ex, K, V, L>
where
    K: 'ex,
    V: 'ex,
    L: LockPolicy + 'ex,
...
impl<'ex, K, V, L> DerefMut for LockGuard<'ex, K, V, L>
where
    K: 'ex,
    V: 'ex,
    L: LockPolicy + 'ex,
