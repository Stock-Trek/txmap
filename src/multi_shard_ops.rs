...
impl MultiShardOps {
        ...
        S: BuildHasher,
    {
        ...
        if let Some(entry) = removed {
            ShardOps::insert::<K, V, S>(shard_to, key_to, entry.1, indexer);
        } else {
        ...
        match a {
            Some((a_key, a_value)) => match b {
                Some((b_key, b_value)) => {
                    {
                        let shard = Self::shard::<K, V, L>(write_guards, key_a);
                        ShardOps::insert_with_duplicate_key(shard, key_a, a_key, b_value, indexer);
                    }
                    {
                        let shard = Self::shard::<K, V, L>(write_guards, key_b);
                        ShardOps::insert_with_duplicate_key(shard, key_b, b_key, a_value, indexer);
                    }
                }
                None => {
                    let shard = Self::shard::<K, V, L>(write_guards, key_b);
                    ShardOps::insert::<K, V, S>(shard, key_b, a_value, indexer);
                }
            },
            None => {
                if let Some((_, b_value)) = b {
                    {
                        let shard = Self::shard::<K, V, L>(write_guards, key_a);
                        ShardOps::insert::<K, V, S>(shard, key_a, b_value, indexer);
                    }
                }
            }
        }
