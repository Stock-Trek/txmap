use crate::{
    indexer::Indexer, key::TxKey, result::MISSING_LOCK_GUARD_ERROR, shard::Shard,
    shard_ops::ShardOps,
};
...
impl MultiShardOps {
    ...
    pub fn move_value<K, V, S>(
        write_guards: &mut [Option<&mut Shard<K, V>>],
        key_from: &TxKey<K>,
        key_to: &TxKey<K>,
        indexer: &Indexer<S>,
    ) where
    ...
    {
        let removed = {
            let shard = Self::shard::<K, V>(write_guards, key_from);
    ...
    {
        let a = {
            let shard = Self::shard::<K, V>(write_guards, key_a);
            ShardOps::remove_entry::<K, V>(shard, key_a.hash_code, &key_a.key)
        };
        let b = {
            let shard = Self::shard::<K, V>(write_guards, key_b);
    ...
    #[inline]
    fn shard<'ex, K, V>(
        write_guards: &'ex mut [Option<&mut Shard<K, V>>],
        key: &TxKey<K>,
    ) -> &'ex mut Shard<K, V> {
