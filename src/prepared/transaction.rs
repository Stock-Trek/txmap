...
/// ```
///
/// The schema carries the map's value type, so storing a transaction only
/// requires writing the schema with its key and value types. The lock policy
...
/// that bakes the schema in, e.g. `DemoPreparedTransaction<'tx, String, u64>`.
pub struct PreparedTransaction<'tx, SCHEMA, L = MutexPolicy, S = DefaultBuildHasher>
where
    SCHEMA: TxSchema + 'tx,
    L: LockPolicy,
    S: BuildHasher,
{
    pub(crate) custodian: &'tx Custodian<SCHEMA::Key, SCHEMA::Value, L>,
...
impl<'tx, SCHEMA, L, S> PreparedTransaction<'tx, SCHEMA, L, S>
where
    SCHEMA: TxSchema + 'tx,
    L: LockPolicy,
    S: BuildHasher,
