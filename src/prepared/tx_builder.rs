...
/// associated types of the schema. The [`tx_schema`](macro@crate::tx_schema)
/// macro generates a specialised builder for each schema
/// (e.g. `TransferPreparedTxBuilder`) that bakes the schema in, so only the
/// map's key and value types need to be written.
    ...
    SCHEMA::Keys: TxKeys<SCHEMA::Key, SCHEMA::IndexedKeys, S>,
{
    ...
    ///
    /// The transaction captures the entire execution plan — the map
    /// references, guards and operations — so the returned
    /// [`PreparedTransaction`] carries only the schema as a generic parameter
    /// and can be executed repeatedly and stored easily.
