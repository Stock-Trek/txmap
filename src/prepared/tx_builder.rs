...
///
/// Start with [`TxMap::prepared_tx`](crate::tx_map::TxMap::prepared_tx) or the
/// schema's specialised `prepared_tx` method generated by the
/// [`tx_schema`](macro@crate::tx_schema) macro (e.g. `Transfer::prepared_tx`),
/// add guard requirements with [`require`](PreparedTxBuilder::require), add
/// operations (e.g. [`modify`](PreparedTxBuilder::modify)), then call
/// [`into_transaction`](PreparedTxBuilder::into_transaction) to obtain a
/// [`PreparedTransaction`] that can be executed repeatedly.
///
/// The map's value type, lock policy and hasher are carried by the schema
/// (value) and by the builder's defaulted generics (lock policy and hasher);
/// the keys, params and state types are associated types of the schema. The
/// [`tx_schema`](macro@crate::tx_schema) macro generates a specialised
/// builder for each schema (e.g. `TransferPreparedTxBuilder`) that bakes the
/// schema in, so only the map's key and value types need to be written.
...
impl<'tx, SCHEMA, L, S> PreparedTxBuilder<'tx, SCHEMA, L, S, PreparedBuilderPhase>
where
    SCHEMA: TxSchema + 'tx,
    L: LockPolicy,
    S: BuildHasher,
...
impl<'tx, SCHEMA, L, S, PHASE> PreparedTxBuilder<'tx, SCHEMA, L, S, PHASE>
where
    SCHEMA: TxSchema + 'tx,
    L: LockPolicy,
    S: BuildHasher,
{
    ...
    ) -> PreparedTxBuilder<'tx, SCHEMA, L, S, PreparedBuildablePhase> {
...
impl<'tx, SCHEMA, L, S> PreparedTxBuilder<'tx, SCHEMA, L, S, PreparedBuildablePhase>
where
    SCHEMA: TxSchema + 'tx,
    L: LockPolicy,
    S: BuildHasher,
    ...
    SCHEMA::Keys: TxKeys<SCHEMA::Key, SCHEMA::IndexedKeys, S>,
{
    ...
    /// references, guards and operations — as its own fields, so no closure
    /// needs to be boxed. The returned [`PreparedTransaction`] is generic
    /// over the schema (plus defaulted lock policy and hasher) and can be
    /// executed repeatedly and stored easily.
    pub fn into_transaction(self) -> PreparedTransaction<'tx, SCHEMA, L, S> {
