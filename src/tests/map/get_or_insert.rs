...
fn get_or_insert_inserts_when_absent() {
    let map = empty_map();
    let returned = map.get_or_insert(&ALICE.into(), 42);
...
fn get_or_insert_with_inserts_when_absent() {
    let map = empty_map();
    let returned = map.get_or_insert_with(&ALICE.into(), |key| key.len() as u64);
...
fn get_or_insert_with_returns_existing_value() {
    let map = map_alice(1);
    let returned = map.get_or_insert_with(&ALICE.into(), |_key| 42);
    assert_eq!(returned, 1);
    assert_eq!(map.get_copied(&ALICE.into()), Some(1));
}

#[test]
fn get_or_insert_with_cloned_value_type() {
    let map: crate::prelude::TxMap<String, String> = empty_typed_map();
    let returned = map.get_or_insert(&ALICE.into(), "default".into());
    assert_eq!(returned, "default");
    let returned = map.get_or_insert(&ALICE.into(), "other".into());
    assert_eq!(returned, "default");
