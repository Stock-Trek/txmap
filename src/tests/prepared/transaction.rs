...
/// `TransferPreparedTransaction` type is generated by `tx_schema!` and bakes
/// the schema in, so the lock policy and hasher default to `MutexPolicy` and
/// `DefaultBuildHasher`.
#[test]
...
/// generic parameter: the schema instantiated with its key and value types.
#[test]
fn prepared_transaction_is_storable_with_schema_generic() {
    struct App<'tx> {
        increment: PreparedTransaction<'tx, Increment<String, u64>>,
    }
    impl<'tx> App<'tx> {
        fn new(map: &'tx TxMap<String, u64>) -> App<'tx> {
            App {
...
fn specialised_prepared_tx_method_needs_no_generics() {
    ...
    let builder: TransferPreparedTxBuilder<'_, String, u64> = Transfer::prepared_tx(&map);
    let tx = builder
        .modify(Transfer::from, |_k, v, p, _s| *v -= p.amount)
        .modify(Transfer::to, |_k, v, p, _s| *v += p.amount)
        .into_transaction();
