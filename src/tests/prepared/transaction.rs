...
/// schema instantiated with its key and value types. The specialised
/// `TransferPreparedTransaction` type is generated by `tx_schema!` and is
/// generic over the schema, so the lock policy and hasher default to
/// `MutexPolicy` and `DefaultBuildHasher`.
#[test]
fn prepared_transaction_is_storable_with_one_generic() {
    struct App<'tx> {
        transfer: TransferPreparedTransaction<'tx, Transfer<String, u64>>,
    }
    impl<'tx> App<'tx> {
...
/// The macro-generated `IncrementPreparedTransaction` type is storable with a
/// single generic parameter: the schema instantiated with its key and value
/// types. There is no standalone generic `PreparedTransaction` type anymore;
/// every schema gets its own macro-generated type.
#[test]
fn prepared_transaction_is_storable_with_schema_generic() {
    struct App<'tx> {
        increment: IncrementPreparedTransaction<'tx, Increment<String, u64>>,
    }
    impl<'tx> App<'tx> {
        fn new(map: &'tx TxMap<String, u64>) -> App<'tx> {
            App {
