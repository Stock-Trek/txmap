...
/// A prepared transaction can be stored with a single generic parameter: the
/// schema's key type. The specialised `TransferPreparedTransaction` type is
/// generated by `tx_schema!` and erases the map's value type, lock policy
/// and hasher.
#[test]
fn prepared_transaction_is_storable_with_one_generic() {
    struct App<'tx> {
        transfer: TransferPreparedTransaction<'tx, String>,
    }
    impl<'tx> App<'tx> {
        fn new(map: &'tx TxMap<String, u64>) -> App<'tx> {
            App {
                transfer: transfer_prepared_tx(map)
...
/// The generic `PreparedTransaction` type is also storable with a single
/// generic parameter: the schema instantiated with its key type.
#[test]
fn prepared_transaction_is_storable_with_schema_generic() {
    struct App<'tx> {
        increment: PreparedTransaction<'tx, Increment<String>>,
    }
    impl<'tx> App<'tx> {
        fn new(map: &'tx TxMap<String, u64>) -> App<'tx> {
            App {
...
/// The specialised `prepared_tx` function generated by `tx_schema!` needs no
/// generic parameters at the call site: every type is inferred from the map.
/// The returned builder is the specialised `TransferPreparedTxBuilder` type.
#[test]
fn specialised_prepared_tx_function_needs_no_generics() {
    let map = empty_map();
    map.insert(ALICE.into(), 100);
    map.insert(BOB.into(), 0);

    let builder: TransferPreparedTxBuilder<'_, String, u64> = transfer_prepared_tx(&map);
    let tx = builder
        .modify(Transfer::from, |_k, v, p, _s| *v -= p.amount)
        .modify(Transfer::to, |_k, v, p, _s| *v += p.amount)
        .into_transaction();
