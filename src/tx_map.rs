    ...
    S: BuildHasher,
{
    ...
    /// Internal store of the map's shards. Exposed (hidden from docs) so the
    /// specialised prepared transaction types generated by
    /// [`tx_schema`](macro@crate::tx_schema) can access the map's internals.
    #[doc(hidden)]
    pub custodian: Custodian<K, V, L>,
    /// The map's key hasher. Exposed (hidden from docs) so the specialised
    /// prepared transaction types generated by
    /// [`tx_schema`](macro@crate::tx_schema) can access the map's internals.
    #[doc(hidden)]
    ...
    S: BuildHasher,
{
    ...
    /// The returned builder is the schema's specialised `PreparedTxBuilder`
    /// (e.g. `TransferPreparedTxBuilder`) and can be turned into a
    /// `PreparedTransaction` (e.g. `TransferPreparedTransaction`) that can be
